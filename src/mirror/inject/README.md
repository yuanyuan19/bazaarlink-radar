...
1. **数据源只有本地 SQLite**。镜像进程跟官方页面同一套节奏盯 `/api/probe/active`（有任务 5s、空闲 10s）；某个 runId 从 active 里消失即刻拉一次 `history` 入库，完成的记录几秒内进库。旧的自适应间隔轮询只做兜底。官方窗口 100 条上限不再构成风险，列表严格只增不减。
2. **首屏**：官方请求 `/api/probe/history?limit=50` 被 `perf.js` 改写为 `/__bl/history-page.json?page=&q=&band=&asOf=`，纯 SQL 分页，每项与官方接口同形（`score` 0–100、`identityOnly`、`totalProbes`），官方组件原样渲染。
...
6. **进行中**：官方每次渲染拼在列表最前，第 2 页起用 CSS 隐掉，只在列表开头出现。
7. **不做**：不克隆表格、不虚拟滚动、不自己画行。
